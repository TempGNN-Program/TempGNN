...
sanity evidence. Artifact \texttt{A2} additionally executes four distinct xclbins and
...
The test command must finish with \texttt{Ran 28 tests} and \texttt{OK}. Optional Q14 outputs
...
The checked parameters are in \path{configs/u280_core_reproduction.json}: requested
...
The package includes smoke run \path{pap142_u280_smoke_20260715T052052Z} and final

run \path{pap142_u280_measured_20260715T052052Z}; the latter contains 288 measured
...
The sole numeric input is \path{reference_inputs/paper_figure_values.csv} with 668

source-labeled rows. \path{reference_inputs/README.md} records source hashes,
...
\path{figure_data_manifest.csv}. The manifest identifies the input file and data
...
the same \path{bin/} directories. Source, paper-mechanism mapping, and limitations
...
\path{results/reviewer_u280_runs/pap142_u280_measured_20260715T052052Z/}; its four

raw CSV files contain 72 rows each (288 total). The packaged one-repetition

smoke run is \path{pap142_u280_smoke_20260715T052052Z}.
...
differ by more than 0.5 MHz. \texttt{verification.md} reports the maximum relative error against
...
\texttt{make smoke} includes the 28 tests from \texttt{A1}.
...
packaged paper plotting records for \texttt{C4}. It does not demonstrate a fresh rerun
